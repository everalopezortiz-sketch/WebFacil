...
export async function handleDiagnosticsRoute(ctx) {
  ...
  try {
    ...
    // ===== FIELD OPTIONS (save custom option for future forms) =====
    if (sub === 'field-options' && method === 'POST') {
    ...
    if (sub === 'records') {
      ...
      if (!id && method === 'POST') {
        ...
        const args = {
          ...
          p_status: status,
          p_record_id: isUuid(body.record_id) ? body.record_id : null,
        ...
        const { data, error } = await supabaseAdmin.rpc('save_diagnostic_bundle', args)
