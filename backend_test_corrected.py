#!/usr/bin/env python3
"""
WebBuilder SaaS API Testing Suite - Corrected Version
Tests all backend API endpoints with proper schema-aware data
"""
...
class APITester:
    ...
    def test_user_settings(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                # Test POST settings (update) - use only valid fields
                update_data = {"currency": "EUR"}  # Only use currency which exists in schema
                response = self.user_session.post(
                    f"{BASE_URL}/settings",
                    json=update_data,
                    timeout=10
                ...
                if response.status_code == 200:
                    ...
                    self.log_result("Update User Settings", False, f"Settings update failed with status {response.status_code}", response.text)
                    return False
            else:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                new_product = {
                    ...
                    "price": 29.99,
                    "is_active": True
                    # Removed stock_quantity as it doesn't exist in schema
                }
    ...
    def run_all_tests(self):
        """Run all API tests"""
        print(f"🚀 Starting WebBuilder SaaS API Tests (Corrected)")
...
def main():
    ...
    with open('/app/test_reports/backend_api_results_corrected.json', 'w') as f:
        json.dump(tester.test_results, f, indent=2)
    
    print(f"\n📄 Detailed results saved to: /app/test_reports/backend_api_results_corrected.json")
